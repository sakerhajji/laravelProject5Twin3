...
@section('content')
    <div class="container">
        ...
        <form id="maladieEditForm" action="{{ route('maladies.update', $maladie->id) }}" method="POST" novalidate>
            @csrf
            @method('PUT')
            <div class="form-group mb-2">
                <label for="nom">Nom</label>
                <input type="text" name="nom" class="form-control" id="nom" value="{{ old('nom', $maladie->nom) }}" required>
            </div>
            <div class="form-group mb-2">
                <label for="description">Description</label>
                <textarea name="description" class="form-control" id="description">{{ old('description', $maladie->description) }}</textarea>
            </div>
...
@section('scripts')
    <script>
        ...
        document.getElementById('maladieEditForm').addEventListener('submit', function(e) {
