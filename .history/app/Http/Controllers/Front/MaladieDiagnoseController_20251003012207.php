    ...
    public function saveMaladie(Request $request)
    {
        ...
        if ($maladieId && $user) {
            try {
                ...
                if ($e->getCode() == 23000) {
                    return redirect()->route('front.maladie.diagnose')->with('error', 'Cette maladie est déjà enregistrée dans votre historique.');
                }
                throw $e;
            }
        }
        return redirect()->route('front.maladie.diagnose')->with('error', 'Impossible d’enregistrer la maladie.');
    }
