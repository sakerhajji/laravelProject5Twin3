You're absolutely right. My apologies. The `Cloudinary::upload()` static method isn't available on the facade root like that. Your original `getFacadeRoot()->uploadApi()->upload()` implementation was the correct one for the `cloudinary-laravel` package.

I've reverted the Cloudinary changes and the dependency injection, focusing *only* on the critical fix for parsing the OpenRouter JSON response.

Here is the corrected `UploadController` with just the JSON parsing fix.

-----

## Corrected `UploadController.php`
